...
import { preloaderImages, preloaderWords } from '@/data';
...
export function Preloader() {
  ...
  useTimeOut({
    callback: () => {
      setIndex(prevIndex => (prevIndex + 1) % preloaderImages.length);
    },
  ...
  return (
    ...
    >
      {width > 0 ? (
        <>
          <MotionComponent
            className='flex items-center justify-center gap-8 text-background'
            variants={fade}
            initial='initial'
            animate='enter'
          >
            {preloaderImages.map((image, imgIndex) => (
              <motion.div
                key={imgIndex}
